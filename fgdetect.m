...
subplot(2,2,2); plot(1:nFrames,A(1)); title('Pixel at (100,100)'); 
xlabel('frame'); ylabel('intensity');
subplot(2,2,3); plot(1:nFrames,B(1)); title('Pixel at (100,200)'); 
xlabel('frame'); ylabel('intensity');
subplot(2,2,4); plot(1:nFrames,C(1)); title('Pixel at (250,200)'); 
xlabel('frame'); ylabel('intensity');
set(findall(gcf,'type','text'),'fontSize',16,'fontWeight','bold')

for k=2:nFrames
    subplot(2,2,1); imshow(video(:,:,:,k));
hold on; plot([100 100 250],[100 200 200],'ro'); hold off;
subplot(2,2,2); plot(1:k,A(:,1:k));
subplot(2,2,3); plot(1:k,B(:,1:k));
subplot(2,2,4); plot(1:k,C(:,1:k));
end